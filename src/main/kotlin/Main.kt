...
// - Создайте класс `clas.Vehicle` с базовыми свойствами, такими как `speed` и `color`.
// - Реализуйте два подкласса `Car` и `Bike`, наследующие класс `clas.Vehicle`. Добавьте в эти классы уникальные свойства и методы (например, `numOfWheels` для `Bike`).

//2. Методы и Переопределение:
// - Добавьте метод `displayInfo()` в класс `clas.Vehicle`, который выводит информацию о транспортном средстве.
...
//- Сделайте класс `clas.Vehicle` абстрактным и добавьте в него абстрактный метод `makeSound()`.
...
//7. Полиморфизм:
//   - Создайте массив или список объектов `clas.Vehicle`, содержащий различные экземпляры `Car`, `Bike`, `ElectricCar` и так далее.
